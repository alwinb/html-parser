...
class TestRunner {
  ...
  run (amount = Infinity) {
    ...
    let skip = 0, fail = 0, pass = 0, total = 0

    for (let input of this.tests) {
      if (total > amount) return
      else total ++
    if (!this._filter (input)) skip++
    else {
      let output, error, assertionFailures = []
      try { output = this.runTest (input) }
      catch (e) { error = e; output = e }
    
      for (let { description, predicate } of this.assertions) {
    ...
    log ('\n\n------------ Results -------------')
    log ('skipped ' + skip + ' of ' + total + ' tests')
    total = fail + pass
    log ('passed  ' + pass + ' of ' + total + ' tests')
