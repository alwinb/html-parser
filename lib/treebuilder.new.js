...
const childRules = {
  ...
  html: {
    ...
    content: head | body,
    openFor: Any,
      paths: { _default: 'body', link:'head', title:head }
  },
...
class Frame {
  ...
  getRule (name, idname, closeCount) {
    log ('getRule', this.name, name, idname, closeCount, head)
    if (this.idname & html){//} && closeCount & head) {
      return {
        closable: None,
        escalate: None,
        content: head | body,
    ...
    //   log ('afterHead!')
    // }
    return childRules [name] || childRules._default
  }
...
log (util.inspect(b.document, {depth:100}))
