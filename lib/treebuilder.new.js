...
function TreeBuilder ({ root = new DocumentFrame (), minDepth = 2, verbose = false } = { }) {
  ...
  const self = this
  // const root = new DocumentFrame ()
  const stack = this.stack = [root]
  let tip = root
  this.document = root.node
  this.closeCount = 0n
  hidden (this, { root, minDepth:2, open, prepare, append, close, lookup, log })
...
function DocumentFrame () {
  const context = { content: html, openFor:Any, paths: { '#default':'html' } }
  return new Frame ('#document', None, context)
}

// Used to collect table contents
function BodyBuilder () {
  builder = new TreeBuilder ({ root: new DocumentFrame () }) .open ('body')
  builder.document = builder.document.children[0].children[1]
...
function Director () {
  ...
  hidden (this, { batchWrite, builder })
  return this
  ...
  function batchWrite (tokens) {
    for (const tag of tokens)  { 
      context = builder.stack [builder.stack.length - 1]
    ...
    if (_tag === START) {
      if (kind & E.table) {
        if (target.prepare (item, kind)) {
          const collector = BodyBuilder ()
          target.append (collector.document) .open (item, kind)
          const frame = target.stack [target.stack.length-1]
