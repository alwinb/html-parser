...
class Frame {
  ...
  child (name, kind, node) {
    // if (kind & this.content) {
      ...
      // HACK
      if (kind & C.void)
        content = openFor = redirect = allow = None, escalate = Any
...
function direct (tokens) {
  ...
  let redirected = new TreeBuilder ()
  ...
  for (const tag of tokens)  { 
    ...
    if (tag.substring (0, 2) === '</') {
      ...
      // TODO pass through the annotator / 
      // to derive the kind, and then, adjust the kind
      ...
      if (name === 'p' && !(context.closable & E.p)) { // FIXME
        builder. open (name) .close (name)
        continue
      }
    ...
    if (tag[0] === '<') {
      ...
      // log (name, context.info)
      if (kind & E.select && context.closable & E.select) { // FIXME
        builder.close (name, kind)
        continue
      }

      // Redirection of misplaced table content

      if (context.redirect & kind && context.closable & E.table) {
        log ('TODO, redirect mispaced table content')
        redirected.open (name, kind)
        continue
        // TODO reset the document for each table,
        // and prepend it, ea.
      }
      
      builder.open (name, kind)
      continue
    }
