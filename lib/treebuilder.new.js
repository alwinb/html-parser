...
class Frame {
  ...
  constructor (name, kind, context = { }, node) {
    const {
      ...
      foster = None, fosterStack = null } = context
  ...
  child (name, kind, node) {
    // if (kind & this.content) {
      let {
        ...
        openFor = None, paths = null,
        foster = None, fosterStack = null,
      } = getRule (this, name, kind, this.children)
      
      // HACK
      if (kind & C.void)
        content = openFor = foster = allow = None, escalate = Any
  ...
  get info () {
    return {
      ...
      foster:    printInfo (this.foster),
      children:  printInfo (this.children),
      // fosterParent: fosterStack && fosterStack[0].node.name,
      // node
    }
  }
