...
function Director ({ context: ctx = documentContext, verbose = false } = { }) {
  ...
  builder._onopen (openHooks, (name, kind, node, nodeParent, nodeIndex) => {
    switch (kind) {
      ...
      case E.head: {
        ...
        builder.stack[1].redirectTo = new TreeBuilder ({ context:ctx, node:ctx.node, getRule, siblingRule, verbose })
        return
      }
      ...
      case E.table: {
        ...
        const parent = new TreeBuilder.Node ('#reparented', None)
  ...
  function* transform (tokens) {
    ...
    for (let i=0; i<l; i++) {
      ...
      // TODO clean up
      const _kind = t & (T.RawText | T.RcData | T.PlainText | T.Comment | T.Bogus) ? C.DATA : t & R.Space ? C.SPACE : C.TEXT
