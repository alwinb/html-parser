...
const { getRule, siblingRule, SVGTagNameAdjustments } = require ('./schema')
...
function Director ({ context: ctx = documentContext, verbose = false } = { }) {
  ...
  const self = this
  let builder = new TreeBuilder ({ context:ctx, verbose, getRule, siblingRule })
  ...
  builder._onopen (E.html|E.head|E.body|E.frameset, (_, kind, node) => {
    // log ('openHook', printKind (kind))
    switch (kind) {
      case E.html: return html = node
      case E.head: return head = node
      case E.body: return body = node
    ...
    for (const [type, item, kind, attrs] of transform (tokens)) {
      ...
      if (type === START) {
        ...
        if (kind & E.head && builder.prepare (item, kind)) {
          ...
          frame.target = new TreeBuilder ({ context:ctx, node:ctx.node, minHeigth:1, getRule, siblingRule, verbose })
        }

        else if (kind & E.table && builder.prepare (item, kind)) {
          // Hmm This is yet more complicated
          const collector = new TreeBuilder ({ context:bodyContext, minHeigth:1, getRule, siblingRule, verbose })
