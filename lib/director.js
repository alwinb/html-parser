...
function Director ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  builder._onopen (openHooks, (name, kind, node, nodeParent, nodeIndex) => {
    switch (kind) {
      ...
      case E.body: return (bodyIndex = nodeIndex, builder.tip.fosterParent = nodeParent, body = node)
  ...
  function* preFilter (tokens) {
    ...
    for (let i=0; i<l; i++) {
      ...
      if (t & START) {
        ...
        switch (kind &( C.startTagExceptions|E.frameset)) {
        ...
        case E.frameset:
          if ((builder.allOpened &~ C.framesetOK) === E.body) {
            ...
            log (printKind (builder.allOpened &~ C.framesetOK))
          }
          break
