...
const { tokenTypes:T, tokenRoles:R, typeName, typeMask, roleMask }
  = require ('./lexer')
...
function Director ({ context: ctx = documentContext, verbose = false } = { }) {
  ...
  builder = new TreeBuilder ({ context:ctx, verbose, getRule, siblingRule })

  const openHooks = E.html | E.head | E.body | E.frameset | E.table
  builder._onopen (openHooks, (_, kind, node) => {
    switch (kind) {
