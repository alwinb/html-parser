...
import { Any, None, Kind, printKind, states as S, C }
  from './categories.js'
...
class Frame {
  ...
  createFrameForChild (name, kind, attrs, node = new Node (name, attrs)) {
    const rule      = childRule (this.state, node)
...
function TreeBuilder ({ context, node, verbose = false }) {
  ...
  function findPathTowards (name, kind) { // to be opened for insert
    ...
    if (kind & tip.rule.openFor)
      while (tip_ && !(tip_.rule.content & kind) && tip_.rule.openFor & kind) {
        const name_ = name in tip_.rule.paths
          ? tip_.rule.paths[name]
          : tip_.rule.paths['#default']
        tip_ = tip_.createFrameForChild (name_, Kind ({name:name_})) // // REVIEW NB not doing preprocessing here!
        path.push (tip_)
  ...
  function tryOpen (name, kind, attrs) {
    // log ('try open ::', name)
  ...
  function _close (index) {
    ...
    while (stack.length > index) {
      ...
      kind = frame.kind
      name = frame.name
      allClosed |= frame.kind
      closes.push (frame)
      formatting.push (frame) 
      // if (closeMask & kind)
