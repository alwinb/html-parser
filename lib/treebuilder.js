...
function TreeBuilder ({ context, node, minDepth = 1, verbose = false }) {
  ...
  function tryClose (name, kind = kinds[name] || otherKind, namespace) {
    // log ('try close ::', name)
    const cursor = canClose (name, kind, namespace)
    if (cursor) {
  ...
  function canClose (name, kind, ns) { // tags to be closed for an endtag
    for (let i=stack.length-1; i>=0; i--) {
      const frame = stack[i]
      // if (frame.name === name && frame.namespace === ns) return { index:i, frame }
