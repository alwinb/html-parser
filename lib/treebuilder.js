...
class Frame {
  ...
  constructor (node, rule) {
    ...
    this.content  = rule.allow // union of eq-classes of allowed child elements (including foster parented)
    this.rule     = rule
  ...
  applyRule (rule, { name, kind, node, children = None } = this) {
    const frame = setProto ({ name, kind, node, children,
      ...
      escalate     : (this.content | this.escalate | this.rule.openFor) & rule.escalate,
      content      : this.content & rule.inherit | rule.allow,
...
function TreeBuilder ({ context, node, minDepth = 1, verbose = false }) {
  ...
  function findParentFor (name, kind) { // to be closed for insert
  ...
  function findPathTowards (name, kind) { // to be opened for insert
    ...
    if (kind & tip.rule.openFor) while (tip_ && !(tip_.content & kind) && tip_.rule.openFor & kind) {
    ...
    for (let i=0; i<3; i++) {
      ...
      if (inserts && inserts.path.length) {
        ...
        for (const tip_ of inserts.path) {
          const parent = kind & tip.rule.reparent ? tip.fosterParent : tip.node
