...
const FrameClass = ({ getRule, siblingRule }) => class Frame {
  ...
  constructor (context = { }, node) {
    const {
      ...
      redirect = None, target = null, parent = null,
      siblingRules = false, _reopen = None, reparent = None } = context
  ...
  applyRule (rule) {
    ...
    const {
      ...
      openFor = None, paths = null,
      redirect = None, target = this.target, parent = this.parent,
      siblingRules = false, _reopen = None, reparent = None,
    } = rule

    const content_ = (content | reparent) &~ forbid | allow
    const frame = setProto ({
      ...
      openFor, paths,
      redirect: redirect &~content_, target, parent,
      siblingRules, _reopen, reparent,
      node,
    }, Frame.prototype)
  ...
  get info () {
    ...
    for (const k in r) if (typeof r[k] === 'bigint')
      r [k] = pr (r [k])
    r.target = this.target?.name// && this.targe.name // t.stack[0].name // && this.target.stack[0].info // && this.target.name
    return r
  }
...
function TreeBuilder ({ context, getRule, siblingRule, node, minDepth = 1, verbose = false }) {
    ...
    for (let i=0; i<3; i++) {
      ...
      if (inserts && inserts.path.length) {
        ...
        for (const tip_ of inserts.path) {
          const parent = tip.reparent & kind ? tip.parent : tip.node
          // log ('foster parent ', parent === node, parent)
  ...
  function tryAppend (item, kind = kinds[item] || otherKind) {
    ...
    if (prepare (item ? item.name : null, kind)) { // NB REVIEW passing item.name here
      const parent = tip.reparent & kind ? tip.parent : tip.node
      // log ('foster parent ', parent === node, parent, stack)
      parent.children.push (item)
      tip.children |= kind // REVIEW, esp. with parent
      allOpened |= kind
      // log ('appended ::', item)
      return self
    }
