...
const TreeBuilderClass = ({ childRule, siblingRule, ruleInfo }) => {
...
function TreeBuilder ({ context, node, minDepth = 1, verbose = false }) {
  ...
  function canClose (name, kind, namespace = null) { // tags to be closed for an endtag
    for (let i=stack.length-1; i>=0; i--) {
      const frame = stack[i]
      // if (frame.name === name && namespace == null || frame.namespace === namespace) return { index:i, frame }
  ...
  function canEscalate (name, kind, namespace) { // to be closed for insert
    if (kind & tip.openable) for (let i=stack.length-1; i >= 0; i--) {
      const frame = stack[i]
      const ns_ok = true//namespace === frame.namespace
