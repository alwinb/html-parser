...
const { Any, None, kinds, otherKind, printKind }
  = require ('./categories')
...
function FrameClass (getRule) {
  ...
  get info () {
    return {
      name: this.name,
      namespace: printKind (this.namespace),
      kind:      printKind (this.kind),
      children:  printKind (this.children),
      closable:  printKind (this.closable),
      openable:  printKind (this.openable),
      content:   printKind (this.content),
      openFor:   printKind (this.openFor), paths: this.paths,
      redirect:  printKind (this.redirect),
...
function TreeBuilder ({ context, getRule, node, minDepth = 1, verbose = false }) {
  ...
  function prepare (name, kind) {
    // log ('try prepare for ::', name, '('+printKind (kind)+')', tip.info)
    for (let i=0; i<3; i++) {
      // log ('loop', i)
      // log ('is allowed ::', name, '('+printKind (kind)+') ?', Boolean (kind & tip.content))// ,'in', tip.info, '?')
      if (kind & tip.content)
        return self
      ...
      if (kind & tip.openFor) {
        ...
        const inserts = pathTowards (name, kind)
  ...
  function tryClose (name, kind = kinds[name] || otherKind) {
    ...
    if (cursor) {
      ...
      if (closes) {
        ...
        // log ('stack length', self.stack.length, 'tip', self.tip.info, self.tip.node)
        return self
      }
    }
  ...
  function pathTowards (name, kind) { // to be opened for insert
    const inserts = []
    let tip_ = tip, ins
