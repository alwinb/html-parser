...
const [ Start, End, Space, Leaf, Warn ]
  = flags ()
...
// ### Create an inverse / naming map;
//  from token types to their (string) names. 
        ...
        do {
          ...
          if (quote && (st|refIn) & Value) { // rows
            [l0, label1, st, sub, quote]
              = char === quote ? [st, Value|End, BeforeAtt, Null, Null]
              : [st, char === AMP ? null : st, Value, Null, quote]
            continue
          }
          ...
          if (st & canEndTag && char === GT) { // cells
            const mark = st & MDecl ? Start : Null;
          ...
          if (st === BeforeValue) { let l // row
            [l0, label1, st, quote] = char & (QUOT|SQUO)
              ...
              : [BeforeValue, char === AMP ? null : Value, Value, Null];
            continue
          }
