...
const [LT, SL, EQ, DQ, SQ, DASH, QM, EM, GT] =
    ...
    while (buffer.length) {
      ...
      for (let i=rest.length, l=input.length; i<l; i++) {
        ...
        do {
          ...
          if (st === Comment) {
            if (c === DASH)
              [labels, sub] = last === DASH ? [null, CommentEnd] : sub !== CommentStart ? [null, null] : [null, sub]
            else if (c === EM && sub === CommentEnd && last !== c)
              labels = null
            else sub = null
          }
        ...
        if (label0 !== label1) {
          if (label0) {
            output[output.length] = ([label0, input.substring (lastEmit, i)])
            if (output.length === n) {
              yield output; output = []
            }
            lastEmit = i
