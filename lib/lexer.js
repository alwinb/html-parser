...
function tokenName (type) {
  return names [type & stateMask] + (type & Start ? 'Start' : type & End ? 'End' : '')
}
          ...
          if (st >= TagName && c === GT) { // cells
            if (tag === EndTag && content !== Data && tagname !== tagname_)
              ...
              [tagname, content]
                ...
                : ['', Data];
              [l0, label1, st, tag, sub] = [st === TagName ? tag|End : st, tag|End, content, null, null];
            }
            continue
          }
...
// log (tokenTypes)
module.exports = { Lexer, chunks, tokenName, tokenTypes, printState }