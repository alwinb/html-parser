...
// I am using a bitfield encoding :flags:types:roles:
// The flags encode part of the trnasition table and 
...
const
  ...
  End   = 1 << 2,
  Space = 1 << 3,
  Warn  = 1 << 4 // NB not exclusive with the others

const tokenRoles =
  { Start, End, Space, Warn }

// 'Flags' are used to encode part of the transition table

const
  _SP   = 1 << 24, // produce Space tokens
  _att  = 1 << 25, // 
  _EQ   = 1 << 26, // responds to '='
  isRef = 1 << 27,  // inside charref
  _GT   = 1 << 28, // responds to '>'
  _TS   = 1 << 29, // responds to 'tagspace'
  _tag  = 1 << 30, // allows tag
  _ref  = 1 << 31 // allows charref

// Token-Types + flags together are used as states

const
  StartTag     = 1 << 5  | _TS ,
  EndTag       = 1 << 6  | _TS ,
  Data         = 1 << 7  | _SP | _tag | _ref,
  RcData       = 1 << 8  | _SP | _tag | _ref,
  RawText      = 1 << 9  | _SP | _tag ,
  PlainText    = 1 << 10 | _SP ,
  Comment      = 1 << 11 | _SP ,
  Bogus        = 1 << 12 | _SP | _GT,
  BeforeAtt    = 1 << 13 | _TS | _att,
  AttName      = 1 << 14 | _TS | _EQ,
  AfterAttName = 1 << 15 | _TS | _att | _EQ,
  BeforeValue  = 1 << 16 | _ref ,
  Value        = 1 << 17 | _ref | _GT,
  NamedRef     = 1 << 18 | isRef | _ref,
  NumRef       = 1 << 19 | isRef | _ref,
  HexRef       = 1 << 20 | isRef | _ref,
  Assign       = 1 << 21 ,      // Not used as a state
  MDecl        = 1 << 22 | _GT,  // Not used as a token type
  Leaf         = 1 << 23 // Not used here, but by the parser; temporary solution

const mask     = (1 << 24) -1
const roleMask = 0b1111
const typeMask = mask &~ roleMask

// NB and just now also out of space!
// Still ok though, I think

const _tokenTypes =  {
  StartTag, EndTag, Data, RcData, RawText, PlainText, 
  Comment, Bogus, Value,
  NamedRef, NumRef, HexRef, MDecl,
  BeforeAtt, AttName, AfterAttName, BeforeValue,
  Assign, 
  Leaf // NB Temporarily added for the parser
}

// Building the map of token-types,
// along with an inverse map from types to their (string) names. 

const tokenTypes = {}, names = {}

for (let k in _tokenTypes) {
  const t = _tokenTypes [k] & typeMask
  tokenTypes [k] = t
  names [t] = k
}
