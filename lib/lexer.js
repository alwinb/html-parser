...
function stateInfo (st) {
  ...
  const { last, quote, tagname, tagname_, line } = st
  const r = { line, last: ch (st.last), quote:ch (st.quote), tagname, tagname_ }
...
function Lexer () {
  ...
  Object.defineProperty (this, 'state', { get: $=> 
    ({ st, sub, content, tag, context, last, quote, tagname, tagname_, label0, line })})
    ...
    while (queue.length) {
      ...
      for (let i=rest.length, l=input.length; i<l; i++) {
        ...
        do {
          ...
          if (st & _ref && last === AMP) {
            if (isAlpha (c)) {
              [context, label1, st] = [st, NamedRef, NamedRef]
              continue
            }
            if (c === HASH) {
              [context, label1, sub] = [st, null, NumRef];
              continue
            }
          }
          ...
          if (st & isRef && c === SEMI) {
            [label1, st, sub, context] = [st, context, null, null]
            continue
          }

          if ((sub||st) & isRef) {
            // NB fallthrough!
            [st, sub, context] = [context, null, null]
          }

          // ### Quoted Attribute Values

          if (quote && (st === Value || context === Value)) { // rows
