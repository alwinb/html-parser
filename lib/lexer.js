...
const _charMap = [
  ...
  [ 8n, ALPHAX, ALPHA0, ALPHA0, _, _, _, _, _, ALPHA0 ],
  [ 0n, _, HEXA, HEXA, HEXA, HEXA, HEXA, HEXA, ALPHA0 ],
  [ 8n, ALPHAX, ALPHA0, ALPHA0, _, _, _, _, _, ALPHA0 ],
  [ 0n, _ ],
]
  ...
  function* batchRead (n = 4) {
    ...
    while (queue.length) {
      ...
      for (let i=rest.length, l=input.length; i<l; i++) {
        ...
        const map2_ = _charMap [ c >> 4n ] || [ 0n, _ ]
        const index = (c + map2_[0]) & 0b1111n
        const len2  = BigInt (map2_.length) - 1n
        const char  = map2_ [ len2 <= index ? len2 : index + 1n ]
