...
// The flags encode part of the transition table and 
...
const
  StartTag     = 1 << 6  | _TS ,
  EndTag       = 1 << 7  | _TS ,
  Data         = 1 << 8  | _SP | _tag | _ref,
  RcData       = 1 << 9  | _SP | _tag | _ref,
  RawText      = 1 << 10 | _SP | _tag ,
  PlainText    = 1 << 11 | _SP ,
  Comment      = 1 << 12 | _SP ,
  Bogus        = 1 << 13 | _SP | _GT,
  BeforeAtt    = 1 << 14 | _TS | _att,
  AttName      = 1 << 15 | _TS | _EQ,
  AfterAttName = 1 << 16 | _TS | _att | _EQ,
  BeforeValue  = 1 << 17 | _ref ,
  Value        = 1 << 18 | _ref | _GT,
  NamedRef     = 1 << 19 | isRef | _ref,
  NumRef       = 1 << 20 | isRef | _ref,
  HexRef       = 1 << 21 | isRef | _ref,
  Assign       = 1 << 22 ,      // Not used as a state
  MDecl        = 1 << 23 | _GT  // Not used as a token type

const mask     = (1 << 24) -1
const roleMask = 0b11111
...
function Lexer () {
  ...
  function* batchRead (n = 4) {
    ...
    while (queue.length) {
      ...
      for (let i=rest.length, l=input.length; i<l; i++) {
        ...
        if (label0 && label0 !== label1) { // Indicates a cut
          output[output.length] = [label0 & mask, input.substring (lastEmit, (lastEmit = i))]
    ...
    if (!writable && rest) {
      ...
      output[output.length] = [label0 & mask, rest]
      yield output
      output = []
    }
  }
