...
function Tokeniser (delegate) {
  ...
  this.write = function write (input) {
    ...
    while (pos < length) {
      ...
      // log ([names[typeId], input.substring(anchor, end)])
      ...
      switch (typeId) {
        ...
        case T.newline:
          ...
          // else if (attr) (attr = null, tokenState = S.BeforeAttribute) // HACK // FIXME newlines in quoted values
        break
        ...
        case T.ampersand: case T.lt: case T.other:
          delegate.writeData (input.substring (anchor, end))
        break
        ...
        case T.attributeSepNL:
          if (attr) (tag.attrs[attr[0]] = attr[1], attr = null)
          tokenState = S.BeforeAttribute;
          lastnl = pos; line++; break
