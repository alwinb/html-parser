...
import { E, C, None } from './categories.js'
...
function Tokeniser (delegate) {
  ...
  this.write = function write (input) {
    ...
    while (pos < length) {
      ...
      // log (tokenState, [names[typeId], input.substring(anchor, end)])
      ...
      switch (typeId) {
        ...
        case T.startTagStart_: {
          ...
          if (typeId === T.startTagStart_) lastStartTag = lastStartTag.toLowerCase ()
        ...
        case T.endTag: {
          ...
          if (typeId === T.endTag_) tagName = tagName.toLowerCase ()
        ...
        case T.endTagStart: {
          ...
          if (typeId === T.endTagStart_) tagName = tagName.toLowerCase ()
        ...
        case T.attributeName: {
          ...
          if (typeId === T.attributeName_) attrName = attrName.toLowerCase () // normlise it if needed
        ...
        case T.tagEnd: {
          ...
          tokenState = tag instanceof Element && !(parserContext & Foreign)
            ? contentMap [lastStartTag] || S.Main
            : lastStartTag === 'plaintext' ? S.PlainText : S.Main
        ...
        case T.nulls: // REVIEW
          if (parserContext & Foreign || tokenState !== S.Main) {
            let replaced = ''
        ...
        case T.space: // TODO get rid of conditional
          if (!tag) {
            // TRYOUT -- In tables, distinguish leading space for foster parenting
            // log ('space followed by', JSON.stringify (input.substring (end, end+3)))
            delegate.writeSpace (input.substring (anchor, end))
          }
        ...
        case T.charRefDecimal: {
          const [base, _start] = typeId === T.charRefHex ? [16, anchor+3] : [10, anchor+2];
