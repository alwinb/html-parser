    ...
    while (pos < length) {
      ...
      switch (typeId) {
        ...
        case T.startTagStart: {
          lastStartTag = decode (input.subarray (anchor+1, end)) .toLowerCase ()
        ...
        case T.endTagStart: {
          const tagName = decode (input.subarray (anchor+2, end)) .toLowerCase ()
        ...
        case T.attributeName: {
          ...
          const attrName = decode (input.subarray (anchor, end))
        ...
        case T.squoted:
        case T.quoted:
          if (attr) attr[1] += decode (input.subarray (anchor, end))
        break;
        ...
        case T.tagEnd: {
          ...
          // log (tag, decode (input.subarray (anchor, end)), input[end-2])
          if (input[end-2] === 0x2f)
        ...
        case T.nulls: // REVIEW
          ...
          if (xmlIsh) {
            ...
            for (let i=0, l=buf.length; i<l; i+=3) { // REVIEW using uft8 for now
              buf[i] = 0xEF
              buf[i+1] = 0xBF
              buf[i+2] = 0xBD
            }
            delegate.writeData (buf)
          }
        break

        case T.space:
          if (!tag) delegate.writeSpace (input.subarray (anchor, end))
          else if (attr) {
        ...
        case T.charRefNamed:
          // quick / temp
          const ref = decode (input.subarray (anchor+1, end-1))
          delegate.writeData (encode ({lt:'<', amp:'&'}[ref] || ''))
        break
      }
      
      // log ([typeId, name, decode(input.subarray (anchor, end))])
      anchor = pos = end
    }
  }
