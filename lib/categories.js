...
C.block = // Hmm object not in there (it's allowed in a p)
  E.div | E.address | E.li | C.list | C.h1_h6 | C.dddt | E.button |
  E.p | E.form | C._block1 | C.section | C.pre | E.xmp
...
C.breakout = 
  ...
  C.otherFmt | E.nobr | C.dddt | C.h1_h6 | C.list | E.embed | E.img | E.hr | E.font // NB font should be excluded if it has a size, color or face attribute
...
C.Flow = // goes into <body>, <caption>, <td>, <object>, <div>, <ul>, block-elements alike
  ~ ( E.html | E.body | E.head | E.frameset | E.frame | C.DOCTYPE | C.Tabular | C.annotationXml | C.annotationHtml | C.otherXml )

C.Phrasing = // goes into <p> elements
  (C.Flow &~ C.block) | E.table | E.button
...
function Kind ({ name, attrs }, state = S.main) {
  ...
  if (state & S.inMath) {
    ...
    else return kind & (E.math | E.svg | C.annotationXml | C.annotationHtml |  C.breakout | C.COMMENT | C.TEXT | C.SPACE) ? kind : C.otherXml
  }
  ...
  // In html
    ...
    return kind & (C.annotationXml | C.annotationHtml | C.otherXml) ? C.otherHtml : kind // This is a bit shitty, the nemaspace dependent categorisation, but let's go...
