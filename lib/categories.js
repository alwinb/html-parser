...
const _cats = {
  ...
  embedHtml: `foreignobject desc mi mo mn ms mtext`, // TODO
...
singletons.annotationXml = singletons ['annotation-xml']
...
C.block = // Hmm object and button not in there (are allowed in p)
...
C.breakout = 
  ...
  C.otherFmt | E.nobr | E.embed | E.img | E.hr | E.font // NB font should be excluded if it has a size, color or face attribute
...
C.Flow = // goes into <body>, <caption>, <td>, <object>, <div>, <ul>, block-elements alike
  ~ ( E.html | E.body | E.head | E.frameset | E.frame | C.DOCTYPE | C.Tabular | E.annotationXml | C.embedHtml | C.otherXml )

C.Phrasing = // goes into <p> elements
  C.Flow &~ C.block

C.Foreign =
  E.math | E.svg | E.annotationXml | C.embedHtml | C.otherXml // | C.SPACE | C.TEXT | C.COMMENT
...
// So I think it makes sense to have embedHtml, embeddedXml classes for
...
function Kind ({ name, attrs }, state = S.main) {
  ...
  if (state & S.inMath) {
    if (kind === E.annotationXml && attrs && attrs.encoding) {
      const v = attrs.encoding.toLowerCase ()
      if (v === 'text/html' || v === 'application/xhtml+xml')
      return C.embedHtml
    }
    else if (kind === C.embedHtml && name !== 'foreignobject' && name !== 'desc')
      return C.embedHtml

    else return kind & (E.math | E.svg | E.annotationXml | C.breakout | C.COMMENT | C.TEXT | C.SPACE) ? kind : C.otherXml
  }
  ...
  // in svg
    ...
    if (kind === E.font) {
      ...
      return kind & (E.svg | E.math | C.breakout | C.COMMENT | C.TEXT | C.SPACE) ? kind :
        (kind & E.title) || (kind & C.embedHtml && (name === 'desc' || name === 'foreignobject'))
          ? C.embedHtml // hack... need a separate kind for embedHtml
          : C.otherXml
    }
  }
  ...
  // In html
    ...
    return kind & (E.annotationXml | C.embedHtml | C.otherXml) ? C.otherHtml : kind // This is a bit shitty, the nemaspace dependent categorisation, but let's go...
