...
const _els =
  ...
  script style template div button p li br
...
C.block =
  E.div | E.li | C.list | C.h1_h6 | C.dddt | E.button |
...
C.Flow = // goes into <body>, <caption>, <td>, <object>, <div>, <ul>, block-elements alike
  ~ ( E.html | E.body | E.head | E.frameset | E.frame | C.RAW | C.DOCTYPE | C.Tabular | C.otherXml )
...
function Kind ({ name, attrs }, state = S.main) {
  ...
  if (state & S.inMath) {
    ...
    else return kind & (E.math | E.svg | C.annotationXml | C.breakout | C.RAW | C.COMMENT | C.TEXT | C.SPACE) ? kind : C.otherXml
  }
  ...
  // in svg
    ...
    if (kind === E.font) {
      ...
      return kind & (E.svg | E.math | C.breakout | C.RAW | C.COMMENT | C.TEXT | C.SPACE) ? kind : C.otherXml
    }
  }
