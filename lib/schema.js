...
const svgRule = {
  ...
  escalate: C.breakout, // REVIEW this means it will escalate on <table>
  content: ~C.breakout,
}
...
const embeddedHtmlInTable = {
  namespace: None, // NB. note that most all other rules disallow closing svg elements!
  allowEnd: E.svg | E.math | _tableIsh,  
  escalate: C.tableIsh,
  content: C.bodyContent,
}
...
function getRule ({ namespace, closable }, name, kind) {
  ...
  if (namespace & E.svg)
    return closable & E.table && name === 'desc'
      ? embeddedHtmlInTable
      : svgRules [name] || svgRule
  ...
  if (kind & E.svg)
    return closable & E.table ? svgInTable : svgRule

  if (kind & E.select)
    return closable & E.table ? selectInTable : selectRule
