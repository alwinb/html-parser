...
const afterHeadRule = {
  ...
  // Hmm this is interesting: Mere token redirection is not enough.
  // For e.g. </head><title>Foo --- will fail to redirect "foo"
  // but it also should not redirect "bar" in e.g. </head>Bar
  // So, the system still needs a change. 
}
...
const mathRule = {
  namespace: E.math,
  allowEnd: C.tableIsh | E.math,
  escalate: C.breakout | C.tableIsh,
  content: ~C.breakout,
}

const dataRule = {
  content: C.SPACE | C.DATA | C.TEXT,
  escalate: Any,
}
...
function getRule ({ namespace, closable, children }, name, kind, allOpened) {
  ...
  // sibling rules
  if (kind & E.html && allOpened & E.frameset)
    return afterFramesetRule

  // TODO, this is not sufficient; it gets reopened too.
  // if (kind & E.html && children & E.body) {
  //   return afterBodyRule
  // }

  else if (kind & E.html && allOpened & E.head)
