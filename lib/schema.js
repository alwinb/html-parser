...
const { E, C, printInfo } = require ('./categories')
const { any:Any, none:None } = C
...
const afterHeadRule = {
  ...
  openFor: ~(C.meta | E.frame | C.SPACE), 
    paths: { '#default':'body' },
  redirect: C.meta // well, space should be redirected to html, C.meta to head ...
}
...
const tableRule = {
  ...
  redirect: ~(tableIsh2 | C.hiddenInput) // REVIEW removing hiddenInput should be automatic?
}
...
const tbodyRule = {
  ...
  redirect: ~(tableIsh2 | C.hiddenInput) // NB likewise
}
...
const mathRules = {
  ...
  svg: foreignRule,
  math: foreignRule,
  'annotation-xml': foreignRule //foreignForeignRule,
...
function getRule ({ namespace, closable }, name, kind, allSeen) {
  ...
  // log (`getRule for ${name} in ${this.name}, seen: ${printInfo (allSeen)}`)

  if (namespace & E.math)
    return kind & C.annotationHtml ? embeddedHtmlRule
      : mathRules [name] || foreignRule

  if (namespace)
    return svgRules [name] || foreignRule
