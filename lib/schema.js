...
function childRule (context, name, kind) { let rule
  ...
  if (context & S.inMath)
    return kind & C.annotationHtml
      ? (context & S.inTable ? embeddedHtmlInTable : inEmbeddedHtml)
      : mathRules [name] ?? inMath

  if (context & S.inSvgInTable)
    return svgInTableRules [name] ?? inSvgInTable
