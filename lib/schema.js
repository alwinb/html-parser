...
const _info = {
  ...
  'button':            C.special | B.li | B.pgroup,
  ...
  'frameset li':       C.special | B.li,
  'option':            B.list | B.pgroup | E.option,
  'optgroup':          B.list | B.pgroup | E.optgroup,
  'dialog div':        C.closep,
  ...
  'address dir hr p plaintext xmp':  C.special | C.closep,

  'applet marquee object select': C.special | B.list | B.li | B.pgroup,
...
const defaultBoundarySet = C.special
const _boundarySets = {
  'li': B.list,
  'p': B.pgroup,
...
const rules = {
  ...
  // TODO so this works but it must also be inherited, i.e.
  // any descendents should inherit the 'closeFor' property
  // so that's a bit messy still. 
  // td: tcell,
  // th: tcell,
  ...
  '<afterHead>': {
    ...
    allowed: C.meta | E.script | E.template | E.body | E.frameset , // TODO allowedInHead elements should be added to the head
    paths: { '#default':'body' }
  },
  ...
  select: { 
    allowed: E.option | E.optgroup | E.script | E.template | E.TEXT, // in body?
    recover: 'ignore', 
    closeFor: C.closeS,
    // For input, keygen and textarea, recover is 'close'
    // however this is now implemented by the closeLists
    // ignoreNull: true,
  },
