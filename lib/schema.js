...
function childRule (state, name, kind) {
  ...
  if (state & S.inMath)
    return kind & C.embedHtml     ? R.inEmbeddedHtml
        : kind  & E.annotationXml ? R.inAnnotationXml 
...
// }
// const precompiled = []
// for (let i=0; i<64; i++) precompiled[i] = [printKind(1n<<BigInt(i))]
// for (const k in defaultRules) {
//   let kind = Kind ({name:k}, S.main)
//   let id = log2 (kind)
//   precompiled [id] = [printKind (kind), defaultRules[k]]
// }
//
// log (precompiled)
// process.exit (0)
