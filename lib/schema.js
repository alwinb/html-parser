...
const _cats = 
  `other void meta h1_h6 spec0 spec1 cell closeS scope
  pscope list closeP tbody dddt TEXT` .split (/\s+/g)
...
function getRule (context, name, flags) {
  ...
  if (flags & C.h1_h6) return headingRule
    ...
    return context.scope & E.select ? selectOptgroupRule : mainRules.optgroup
...
function deriv (ctx, node, flags) {
  ...
  const { content = ctx.content, closedBy = ctx.closedBy, paths } = rule
  ...
  return { node, flags, scope, closeFor, closedBy, content, paths }
}
...
module.exports = { 
  ...
  defaultRule, documentRule, afterHeadRule,
  deriv
}