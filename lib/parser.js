...
function TreeBuilder (delegate) {
  ...
  function batchWrite (tags) {
    ...
    // End Tags -- Exceptions
      ...
      // after the head, they are ignored (REVIEW)
    ...
    switch (tag[0]) {
      ...
      break; case T.VoidTag: {
        ...
        if (st) st._arr[0].node.push (new Leaf (tag))
      }

      break; case T.StartTag: {
        log ('start tag', stack.info, '::', tag.name)
        const st = stack.prepareFor (tag, flags)
        if (st) st.open (tag, flags) && _didOpen (flags)
      }
...
function Stack (arr = [], _didOpen) {
  ...
  assign (this, { openerFor, open, close, prepareFor, _arr:stack })
  ...
  function open (tag, flags = elementInfo [tag.name] || defaultInfo) {
    ...
    if (flags & E.table) {
      ...
      frame.foster = new Stack (stack.slice (0), _didOpen)
    }
  ...
  // ### Close
  // Searches for a matching open node.
  // If found, closes all nodes up to and including the matching open node.
