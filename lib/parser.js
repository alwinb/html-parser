...
function TreeBuilder (delegate) {
  ...
  function batchWrite (tags) {
    for (let tag of tags) { log (tag, 'in', self.state); switch (tag[0]) {
      ...
      break; case T.StartTag:
        // Exception: <select> in <select> is treated as </select>
        if (tag.name === 'select' && closeOrIgnore ('select', B.select).length)
          continue;
  ...
  function open (tag, flags = elementInfo[tag.name]||defaultInfo) {
    ...
    if (node instanceof Node) {
      ...
      if (flags & E.head) {
        ...
        stack[stack.length-2][0] = rules['html <after head>']
      }
  ...
  function closeOrIgnore (...closeList) {
    // log ('before closeOrIgnore', stack.map (_ => _[0].name) .reverse())
    let start = 0
