...
function Preprocessor (delegate) {
  ...
  let context = { state:0n, kind:0n, closable:0n, allOpened:0n, allClosed:0n }
  methods (this, { writeTag, writeEndTag, writeSpace, writeData, writeComment, writeDoctype, writeNulls, writeEOF })
    ...
    switch (kind & C.startTagExceptions) {
      ...
      case E.head:
        return context = state & svgOrMath ?
          delegate.writeEndTag (new EndTag (state & E.svg ? 'svg' : 'math'))
          : delegate.writeTag (item)

      // <select> within a <select> is converted to </select>
      case E.select:
        return context = state & E.select && state & E.html ?
      ...
      case E.image:
        return context = delegate.writeTag (
          state & E.html ? new Element ('img') : item)

      // <a> within an <a> is converted to </a><a>
      case E.a:
        if (context.closable & E.a && state & E.html)
          context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item)

      // Likewise for <nobr>
      case E.nobr:
        if (context.closable & E.nobr && state & E.html)
          context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item)

      // <input type=hidden> is distinguished from other <input> tags
      case E.input:
        if (attrs && attrs.type && attrs.type.toLowerCase () === 'hidden')
  ...
  function writeEndTag (item) {
    ...
    if (kind & C.endTagExceptions) {
      ...
      if (kind & E.html) {
        if (allOpened & E.html && !(closable & dontSetMode))
          mode = 'afterAfterBody'
        return context
      }
  ...
  function _adjustTag (item) {
    ...
    if (context.state & E.svg)// || name === 'foreignobject') // REVIEW how to adjust foreignObject endTag?
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  methods (this, { writeTag, writeEndTag, writeData, writeSpace, writeNulls, writeComment, writeDoctype, writeEOF, appendToHtml, appendToDocument })
  return this
  ...
  function feedback (raw = false, _mode = null) {
    ...
    const r = { state, kind, closable, allOpened, allClosed, raw, xmlIsh:!!(closable & svgOrMath) }
  ...
  function writeTag (item, kind = kinds [item.name] || otherKind) {
    ...
    else if (kind & C.RawText && builder.tip.state & E.html) {
