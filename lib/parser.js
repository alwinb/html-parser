  ...
  function endTag (tag, flags) {
    ...
    if (index >= 0) {
      ...
      for (const x of closes) {
        if (x.parent) x.parent.push (x.node)
        if (x.flags & C.format) stack._reopen.unshift (x)
      }
  ...
  function prepareFor (tag, flags) { // tag must be a StartTag or Data
    ...
    for (;;) {
      ...
      if (closeFor >= 0 ? closeFor & flags : !(~closeFor & flags)) {
        ...
        if (frame.flags & C.format) reopen.unshift (frame)
      }
