...
function Kind ({ name, attrs }) {
  ...
  if (kind === E.input && attrs && attrs.type.toLowerCase () === 'hidden')
    return C.hiddenInput

  if (kind === E.font) for (const name in attrs)
    if (name.toLowerCase () in _specialFontAttributes)
      return C.htmlFont
...
function Preprocessor (delegate) {
  ...
  let context = { state:E.html, kind:0n, closable:0n, allOpened:0n, allClosed:0n }
  ...
  function writeTag (item) {
    ...
    switch (kind & C.startTagExceptions) {
      ...
      // FIXME (add attributes, and check... as it doesn't work currently)
      case E.image:
        return context = delegate.writeTag (
          state & E.html ? new Element ('img', item.attrs) : item)

      // <a> within an <a> is converted to </a><a>
      case E.a:
        if (closable & E.a && state & E.html)
          context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item)

      // Likewise for <nobr>
      case E.nobr:
        if (closable & E.nobr && state & E.html)
          context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item)
    }
    ...
    // e.g. foreignObjec, radialGradient, ...
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function feedback (raw = false, _mode = null) {
    ...
    const r = { state, kind, closable, allOpened, allClosed, xmlIsh:!!(closable & svgOrMath) }
