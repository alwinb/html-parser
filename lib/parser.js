...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function writeTag (item, kind = kinds [item.name] || otherKind) {
    ...
    else if (kind & C.RawText && !target.tip.namespace) {
      const success = target.tryOpen (item.name, kind, item.attributes, None)
  ...
  function writeEndTag (item, kind = kinds [item.name] || otherKind) {
    ...
    if (context.namespace === E.html
      && kind & (E.body | E.frameset | E.html)
      && !(context.closable & (E.table | E.select | E.caption | C.cell))) {
  ...
  function writeSpace (buff, _kind = C.SPACE) {
    ...
    target.tryAppend (buff, _kind, null, None)
    return feedback ()
  }
