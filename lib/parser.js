...
function Preprocessor (delegate) {
  ...
  function _adjustTag (item) {
    ...
    if (context.namespace & E.svg)// || name === 'foreignobject') // FIXME how to adjust foreignObject endTag?
      name = SVGTagNameAdjustments [name] || name
    if (item.name === name) return item
    
    if (item instanceof Node) {
      const r = new Node (name, item.attributes)
      r.children = []
  ...
  function writeNulls (buf)  { 
    // Within SVG and MathML U+00 characters are converted to U+FFFD
    // NB assuming UTF8 for now, but that needs to be cleaned up
