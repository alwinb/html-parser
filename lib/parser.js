...
import { E, C, Any, None, kinds, Kind, otherKind, printKind, states as S }
  from './categories.js'
...
const startTagExceptions =
  ...
  | E.table | E.form | E.button | E.li | C.dddt | C.h1_h6 | E.option | E.optgroup | E.p
...
const framesetOK =
  ...
  C.otherVoid | C._block1 | C.section | E.form | C.otherHtml | C.COMMENT | C.SPACE | C.DOCTYPE |
  E.html | E.head | E.body | E.svg | E.math | C.subsup | C.hiddenInput | C.annotationHtml | C.otherXml |
...
function Preprocessor (delegate) {
  ...
  function writeTag (item) {
    ...
    switch (kind & startTagExceptions) {
      ...
      case E.frameset:
        return allOpened &~ framesetOK ?
        context : context = delegate.writeTag (item, E.frameset) // Hmm maybe call a switch method insteads

      // html <image> is converted to <img>
      case E.image:
        return context = delegate.writeTag (new Element ('img', item.attrs), E.img)

      // Tryout specify nestring restrictions here
      // instead of in the schema for a moment
      
      case E.form:
        // TODO need another prop separate from closable...
        if ((nesting & E.form) === 0n)
          context = delegate.writeTag (item, kind)
        return

      // html <a> within an <a> is converted to </a><a>
      case E.a:
        if (nesting & E.a) context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item, E.a)

      // Likewise for <nobr>
      case E.nobr:
        if (nesting & E.nobr)
          context = delegate.writeEndTag (item)
        return context = delegate.writeTag (item, E.nobr)

      // Nesting restrictions -> Escalate
    ...
    // to a start tag immediately followed by an end tag. // FIXME <svg><font/>

    if (item.selfclose && state & svgOrMath && kind &~ (/*C.htmlFont | */C.breakout)) { // REVIEW
      context = delegate.writeTag (item, kind)
  ...
  function writeEndTag (item) {
    ...
    let kind = (kinds [item.name] || C.otherHtml) // REVIEW
    ...
    if (kind &~ (E.svg | E.math | C.breakout)) kind |= C.otherXml
    // REVIEW this is unclear; how should the kind be assigned to end-tags?
    return context = delegate.writeEndTag (item, kind) // NB otherXml
  }
