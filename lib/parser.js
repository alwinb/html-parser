...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function writeTag (item) {
    ...
    if (kind & C.ForeignElement && item.selfclose) {
  ...
  function _writeTag (item, id, kind) {
    ...
    if (kind & C.VoidElement) builder.tryAppend (item, kind) // could be handled by the schema
  ...
  function writeEndTag (item) {
    ...
    if (kind &~ (C.svg | C.math | C.BreakoutElement)) kind |= C.OtherForeign
