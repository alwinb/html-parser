...
function Preprocessor (delegate) {
  ...
  function writeTag (item) {
    ...
    switch (kind & startTagExceptions) {
      ...
      case E.head: {
        if (state & svgOrMath) {
          const [name, kind_] = state & S.inSvg ? ['svg', E.svg] : ['math', E.math]
          return context = delegate.writeEndTag (new EndTag (name), kind_)
        }
        else return context = delegate.writeTag (item, kind)
      }
