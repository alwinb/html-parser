...
const {
  ...
  defaultRule, documentRule, afterHeadRule, deriv } = require ('./schema.js')
...
function TreeBuilder (delegate) {
  ...
  function batchWrite (tags) {
    for (let tag of tags) {
      ...
      if (tag.name === 'br' && type === T.EndTag)
        type = T.StartTag

      // Exception: <select> within <select> is treated as </select>
      if (tag.name === 'select' && stack[0].scope & E.select && type === T.StartTag)
        type = T.EndTag
