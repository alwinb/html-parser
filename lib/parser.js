...
function TreeBuilder (delegate) {
  ...
  function reset () {
    ...
    head = body = null
    stack = [ { rule:rules['#document'], negate:false, allowed:E.html, closeFor:0, node:document, flags:defaultInfo } ]
    afterHead = ''
  }
  ...
  function prepareForInsert (tag, flags) { // tag must be a StartTag or Data
    ...
    let fail = false
    const allowedIn = ({ rule:{name:mname}, allowed:cat = ~0, negate = false }) => {
    ...
    let mode, close
    while (!fail && (mode = stack[0].rule) && ((close = stack[0].closeFor & flags) || !allowedIn (stack[0]))) {
      if (close) closeInScope (stack[0].node.name) // This is just an unconditional close now
      else if ('paths' in mode) {
  ...
  function open (tag, flags = elementInfo[tag.name]||defaultInfo) {
    ...
    if (node instanceof Node) {
      ...
      const rule = stack[0].rule.rules [name] || stack[0].rule.rules ['#default']
      
      let { negate, allowed, closeFor } = stack[0]
      if ('allowed' in rule) {
        negate = rule.negate || false
        allowed = rule.allowed
      }
      closeFor = 'closeFor' in rule ? rule.closeFor
        : 'addCloseFor' in rule ? closeFor | rule.addCloseFor
        : closeFor

      // log ('open tag', { rule, flags, closeFor, node, negate, allowed })
      stack.unshift ({ rule, flags, closeFor, node, negate, allowed })

      // Well, now it's all over the place..
      if (flags & E.head) {
        document.head = head = node
        stack[stack.length-2].rule = rules['<afterHead>'] // Well, upon close anyway
      }
  ...
  function closeInScope (...closeList) {
    log ('before closeInScope', stack.map (_ => _.rule.name) .reverse())
    let start = 0
