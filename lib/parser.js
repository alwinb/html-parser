...
function Kind ({ name, attrs }, state = S.main) {
  ...
  // Quick test; for now marking C.tableIsh as C.other in
  // foreign content (svg and math). TODO Do this properly:
  // Consider all element-names and map to appropriate
  // element equivalence classes (kinds) in each namespace.

  if (state & svgOrMath && kind & C.tableIsh) // NB Tryout
    return otherKind  
...
function Preprocessor (delegate) {
  ...
  function writeEndTag (item) {
    ...
    if (closable & E.svg) // FIXME <svg><foreignObject><math></foreignObject> and alike
      item = _adjustSvgTag (item)
  ...
  function _adjustSvgTag (item) {
    ...
    const r = new Element (name, item.attrs)
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function onopen (name, kind, node, nodeParent, nodeIndex) {
    switch (kind) {
      ...
      case E.frameset: { // Can be done by annotating as C.other in xmlish
  ...
  function writeTag (item, kind = kinds [item.name] || otherKind) {
    ...
    if (kind & C.void)
      ...
      const success = builder.tryOpen (item.name, kind, item.attrs)
      return feedback (true)
    } 
    else
      builder.tryOpen (item.name, kind, item.attrs)
