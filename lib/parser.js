...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function feedback () {
    return { xmlIsh:!!(builder.tip.closable & (E.svg|E.math)) } // FIXME xmlish
  }
  ...
  function writeEndTag (item) {
    ...
    return _writeEndTag (item, kind) // NB otherForeign
  }
  ...
  function writeData (item)  {
    ...
    // console.log ({bufferedSpacesKind:printKind(bufferedSpacesKind), mode}, item)
    if (builder.tip.kind & leadingSpaceAdjust) bufferedSpacesKind = C.TEXT
    _flushSpaces ()
    mode &= clearMode
    builder._reformat ()
