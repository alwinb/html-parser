...
function TreeBuilder (delegate) {
  ...
  function reset () {
    ...
    _arr = [{ node:document, flags:0, visiblyOpen:0, closeFor:0, content, openFor } ]
  ...
  function batchWrite (tags) {
    ...
    switch (tag[0]) {
      ...
      break; case T.FormatTag:
        const st = stack.prepareFor ('', C.format)
  ...
  function endTag (tag, flags) {
    ...
    // else if (!(_arr[0].visiblyOpen & flags)) return // SPEED :D
...
function deriv (frame, node, flags) {
  ...
  const { hideOpen = ~0, content = frame.content, allowEnd = frame.allowEnd, openFor } = rule
