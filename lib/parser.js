  ...
  function batchWrite (tags) {
    ...
    switch (tag[0] & typeMask) {
      ...
      case T.EndTag: {
        ...
        if (index >= 0) stack.close (index, tag.name)
      }
      ...
      break; case T.Data: {
        ...
        const st = stack.prepareFor (tag[1], tag[0] & tokenRoles.Space ? C.SPACE : C.TEXT)
...
function Stack (arr = [], _didOpen) {
  ...
  function prepareFor (tag, flags) { 
    for (;;) {
      ...
      else if (frame.openable & flags) close (0, tag.name)
