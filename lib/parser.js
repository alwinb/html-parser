...
function Preprocessor (delegate) {
  ...
  function writeTag (item) {
    const { state, nesting, allOpened } = context
    ...
    switch (id) {
      ...
      case e.head: {
        if (state & svgOrMath) {
          ...
          return context = delegate.writeEndTag (new EndTag (name), 1n << BigInt (id_))
        }
        else return context = delegate.writeTag (item, id, kind)
      }

      // html <frameset> tags are ignored if particular open tags have been accepted
      case e.frameset:
        return allOpened &~ C.framesetOK ? context
          : context = delegate.writeTag (item, id, E.frameset) // Hmm maybe call a switch method instead

      // html <select> within a <select> is converted to </select>
      case e.select:
        return context = nesting & E.select ?
          delegate.writeEndTag (new EndTag ('select'), E.select) :
          delegate.writeTag (item, id, E.select)

      // html <image> is converted to <img>
      case e.image:
        return context = delegate.writeTag (new Element ('img', item.attrs), e.img, E.img)

      // Nesting restrictions -> Ignore
      
      case e.form:
        if ((nesting & E.form) === None)
          context = delegate.writeTag (item, e.form, kind)
        return context
      ...
      case e.table:
        if (nesting & kind) // close by kind
          delegate.writeEndTag ({ name:null, kind }, kind)
        return context = delegate.writeTag (item, id, kind)
    ...
    if (item.selfclose && kind & C.Foreign) {
      context = delegate.writeTag (item, id, kind)
    ...
    if (kind & endTagExceptions) {
      ...
      if (kind & E.body) {
        ...
        if (!(context.allClosed & E.head) || closable & E.head)
          context = delegate.writeTag (new Element ('body'), e.body, kind)
      ...
      if (kind & E.br)
        return context = delegate.writeTag (new Element ('br'), e.br, kind)
      ...
      if (kind & E.p && !(closable & E.p) && allOpened & E.body) {
        delegate.writeTag (new Element ('p'), e.p, kind) // inserts a token, without attrs
        return context = delegate.writeEndTag (new EndTag (item.name), kind)
      }

      // </head> not within <head> is converted to <head></head>
      if (kind & E.head && !(allOpened & E.head)) {
        delegate.writeTag (new Element ('head'), e.head, kind)
        return context = delegate.writeEndTag (new EndTag ('head'), kind)
      }
    }
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  function feedback () {
    ...
    const r = { state, kind, nesting, closable, allOpened, allClosed, xmlIsh:!!(closable & (E.svg|E.math)) } // FIXME xmlish
  ...
  function writeTag (item, id, kind) {
