...
const { makeTagToken } = require ('./tokens')
const { Document, Leaf, Node } = require ('./dom')
const { E, C, elementInfo, defaultInfo, printInfo } = require ('./categories')
const { documentRule, afterHeadRule, afterFramesetRule, getRule, SVGTagNameAdjustments } = require ('./schema')
const log = console.log.bind (console)

const methods = (obj, dict) => {
  for (const k in dict)
    Object.defineProperty (obj, k, { value:dict[k], enumerable: false })
  return obj
}
...
function TreeBuilder (delegate) {
  ...
  methods (this, { reset, write, batchWrite, end })
  return reset ()
  ...
  function batchWrite (tags) {
    for (let tag of tags) {
      ...
      // Adjust flags for special attributes
      // NB this is safe for input and font, because they are void-tags
      // however, the fact that it works for annotation-xml, is purely a coincidence
      // (The issue is, that re-flagging annotationXml to annotationHTML could cause mismatching end tags)

      const atts = tag.attributes

      if (flags & E.input && (atts.type||'') .toLowerCase () === 'hidden')
        flags = E.hiddenInput
      
      else if (flags & E.font && ('color' in atts || 'face' in atts || 'size' in atts))
        flags = E.htmlFont

      else if (flags & E.annotationXML && 'encoding' in atts) {
        const v = atts.encoding.toLowerCase ()
        if (v === 'text/html' || v === 'application/xhtml+xml')
          flags = E.annotationHTML
      }
      
      // log (tag.name, printInfo (flags))
...
function Stack (arr = [], min = 0) {
  ...
  methods (this, { openerFor, open, reformat, close, prepareFor, _arr:stack })
