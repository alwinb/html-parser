...
function TreeBuilder (delegate) {
  ...
  function batchWrite (tags) {
    for (let tag of tags) { 
      ...
      break; case T.StartTag:
        ...
        if (tag.name === 'select' && closeInScope ('select').length) continue;
  ...
  function endTag (tag, flags) {
    ...
    if (name === 'p' && findInScope ('p', C.pscope) < 0) {
  ...
  function closeForTag (tag, info) { // tag must be a StartTag or data
    ...
    if (info & C.dddt)
       closeInScope (C.dddt, C.scope|C.list | E.li, 'p', C.pscope)

    if (name === 'li')
      closeInScope ('li', C.scope|C.list, 'p', C.pscope)

    if (info & C.heading)
      return closeInScope ('p', C.pscope, C.heading, ~0)
  ...
  function openerFor (search, searchFlags, start = 0) {
    // search is a string (elem name) or a number (elementInfo)
    let index = -1
