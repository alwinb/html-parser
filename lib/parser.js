...
const { Document, Leaf, Node, Mark, UnMark } = require ('../lib/dom')
...
function TreeBuilder (delegate) {
  ...
  function restart () {
    ...
    head = body = null
    stack = new Stack ([document.info, document])
  ...
  function batchWrite (tags) {
    for (let tag of tags) switch (tag[0]) {
      ...
      case T.Data: {
        ...
        if (prepareForInsert (tag[1])) stack[0][1].push (tag[1])
      }
  ...
  function closeForTag (tag) { // tag must be a StartTag or data
    ...
    // log ('closeForTag', self.state, `:: <${name}>`, flags & C.cell)
  ...
  function closeOrIgnore (...args) {
    ...
    for (let i=0, l=args.length; i<l; i+=2) {
      const [search, bounds] = [args[i], args[i+1] || B.document]
...
// The stack contains _pairs_ [infoFlags, node]
