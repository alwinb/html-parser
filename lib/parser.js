  ...
  function writeTag (item) {
    ...
    switch (kind & C.startTagExceptions) {
      ...
      case C.annotationXml:
        if (attrs && state & E.math) for (const { name, value } of attrs)
          if (name === 'encoding') {
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  methods (this, { writeTag, writeEndTag, writeData, writeSpace, writeNulls, writeMDecl, writeEOF, appendToHtml, appendToDocument })
  return this
  ...
  function feedback (raw = false, _mode = null) {
    ...
    const r = { state, kind, closable, allOpened, allClosed, raw }
