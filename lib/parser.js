...
function Stack (arr = [], _hooks, _foster = false) {
  ...
  function prepareForInsert (tag, flags) { // tag must be a StartTag or Data
    ...
    while ((current = stack[0]) && ((close = current.closeFor & flags) || !allowedIn (current))) {
      ...
      if (close && !_foster) {
      ...
      else if (!_foster && paths && (ins = paths[name] || paths['#default'])) {
        ...
        const s = new Stack (stack.slice (i), _hooks, true) .prepareForInsert (tag, flags)
        log ('\tfoster to', s.state, '::', tag.name || '#text ' + tag)
        return s
      }
  ...
  function open (tag, flags = elementInfo [tag.name] || defaultInfo) {
    ...
    if (flags & E.table) frame.parent = stack[0].node
