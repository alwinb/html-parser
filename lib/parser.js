...
const { tokenTypes:T, tokenRoles:R, typeName, typeMask, roleMask } = Lexer
...
function Parser ({ context: ctx = documentRule, verbose = false } = { }) {
  ...
  methods (this, { reset, write, parse, end })
  ...
  function* preFilter (tokens) {
    ...
    for (let i=0; i<l; i++) {
      let kind, [t, name, attrs] = tokens [i]
      ...
      if (r === START) {
        ...
        if (tokens[i].selfclose && namespace & (E.svg|E.math)) {
          ...
          yield kind & C.void ? [LEAF, new Leaf (name, attrs), kind] : [START, name, kind, attrs]
...
Object.assign (Parser, { Parser, Node, Leaf, START, END, LEAF, _private: { C, E } })
